...
export default {
  ...
  activated:function(){
    this.getuserinfo( this.$route.params.yhm,0)
  },
  methods:{
    selectshijiaoFn(e){
      const _this=this
      ...
      if(e.target.selectedIndex==1){
        ...
        _this.getphoto(this.user_id,this.pets[e.target.selectedIndex-2].pet.cwid)
      }
    },
    getphoto(e,t){
      axios.post('http://localhost:8000/getuserfbsjpic?yhid='+e+'&cwid='+t)
      .then((response)=>{
    ...
    getuserinfo(e){
      ...
      axios.get('http://localhost:8000/user/getUserByNamelog/'+e)
        .then(async res=>{
          ...
          _this.getphoto(res.data.yhid,0)
        }).catch(err => {
          console.log('错误！！！！：'+err)
      })
    },
